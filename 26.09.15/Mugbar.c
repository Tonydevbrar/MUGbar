...
static const uint16_t k_bsd[16] ={
    0b0000000000000000,
    0b0000000000000000,
    0b1110001111011100,
    0b1001010000010010,
...
static const uint16_t k_linux[16] ={
    0b0000000001100000,
    0b0000000011110000,
    0b0000000001100000,
    0b0000000000000000,
...
static const uint16_t k_unkn[16] ={
    0b0000111111110000,
    0b0000111111110000,
    0b0011110000111100,
    0b0011110000111100,
...
static const uint16_t k_sun[16] ={
    0b0000000000000000,
    0b0000111001110000,
    0b0000100000010000,
    0b0000001111000000,
...
static const uint16_t k_aix[16] ={
    0b0000000110000000,
    0b0000001111000000,
    0b0000011111100000,
    0b0000111001110000,
...
static const uint16_t k_hpux[16] ={
    0b0000001111100000,
    0b0001001111111000,
    0b0011001111111100,
    0b0110011111111110,
...
static const uint16_t k_haiku[16] ={
    0b0000000000000000,
    0b0000000110000000,
    0b0000001111000000,
    0b0000001111000000,
...
int main() {
    ...
    //# Check system kernel
    char kern;
    if (defined(BSD)) kern="bsd";
    else if(defined(__linux__)) kern="linux";
    else if(defined(__sun)) kern="sun";
    else if(defined(_AIX)) kern="aix";
    else if(defined(__HAIKU__)||defined(__BEOS__)) kern="beos";
    else kern="unknown";
    ...
    while (1) {
        ...
        if (event.type == ButtonPress) {
            ...
            if (click_x >= (swidth-panel_scale)-4 && click_x <= (swidth)) {
                ...
                if (fork() == 0) {
                    ...
                    system(edit_hlink); 
                    _exit(0);}}
            if (click_x >= swidth-(panel_scale*6)-4 && click_x <= swidth-(panel_scale*5)-4) {
                    printf("DEBUG: Попытка запустить команду из конфига: [%s]\n", volu_hlink);
                if (fork() == 0) {
                    setsid();
                    system(volu_hlink); 
        ...
        draw_scalable_binary_icon(draw, &accent_color, audio, swidth-(panel_scale*6)-4, 2, panel_scale-4);
        XftDrawString8(draw, &accent_color, font1, (panel_scale*3)+4, panel_scale*0.8, (unsigned char *)system_string, osinfo);
        //# Net check
        ...
        if (has_internet) draw_scalable_binary_icon(draw, &accent_color, inet, (swidth-(panel_scale*5))-4, 2, panel_scale-4);
        else if (has_network) draw_scalable_binary_icon(draw, &accent_color, net, (swidth-(panel_scale*5))-4, 2, panel_scale-4);
        else {draw_scalable_binary_icon(draw, &accent_color, not, (swidth-(panel_scale*5))-4, 2, panel_scale-4);}
        //# Show time
        XftDrawString8(draw, &accent_color, font1, (swidth-(panel_scale*4))+2, panel_scale*0.8, (XftChar8 *)time_str, strlen(time_str));
        XFlush(display);
