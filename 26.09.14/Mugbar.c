...
void makecfg(){
    ...
    if (access(config_path, F_OK) != 0) {
        ...
        if (default_cfg != NULL) {
            ...
            fprintf(default_cfg, "icons = {term = \"st &\", param = \"st -e vi ~/.config/coffeesys/mugbar.lua &\", vol = \"pavucontrol &\"}");
...
int main() {
    ...
    if (home != NULL) {
        ...
        snprintf(config_path, sizeof(config_path), "%s/.config/coffeesys/mugwm.lua", home);} else {
        strncpy(config_dir, ".config/coffeesys", sizeof(config_dir) - 1);
        strncpy(config_path, ".config/coffeesys/mugwm.lua", sizeof(config_path) - 1);}
    ...
    has_network = (system("ping -c 1 -w 1 8.8.8.8 > /dev/null 2>&1") == 0);
    ping_counter = 1;
    while (1) {
        ...
        if (event.type == ButtonPress) {
            ...
            if (click_x >= 0 && click_x <= (panel_scale+4)) {
                if (fork() == 0) {
                    ...
                    system(term_hlink); 
                    _exit(0);}}
            if (click_x >= (swidth-4-panel_scale) && click_x <= (swidth)) {
                    printf("DEBUG: Попытка запустить команду из конфига: [%s]\n", edit_hlink);
                if (fork() == 0) {
                    setsid();
                    system(edit_hlink); 
                    _exit(0);}}
            if (click_x >= (swidth-4-(panel_scale*5)) && click_x <= (swidth-4-(panel_scale*4))) {
                    printf("DEBUG: Попытка запустить команду из конфига: [%s]\n", volu_hlink);
                if (fork() == 0) {
                    setsid();
                    system(volu_hlink); 
        ...
        if (has_network) {
            draw_scalable_binary_icon(draw, &accent_color, net, (swidth-(panel_scale*4))-4, 2, panel_scale-4);}
        //# Show time
        XftDrawString8(draw, &accent_color, font1, (swidth-(panel_scale*3))+4, panel_scale*0.8, (XftChar8 *)time_str, strlen(time_str));
        XFlush(display);
